...
컨텍스트 : context, 해당 대상의 모든 것, 정보 

'컨텍스트를 넘겨준다'

DB에 저장하기 전에 영속성 컨텍스트 : 자바가 저장하는 모든 데이터를 알고 있다.

영속성 컨텍스트와 DB의 내용은 동기화된다. 

자바에서 데이터를 바뀌면 영속성 컨텍스트가 똑같이 바뀜 

그리고 영속성 컨텍스트에서 DB로 넣어주려하면 인서트가 아니라 업데이트가 일어남

영속성 컨텍스트는 모든 메타 데이터를 가지고 있음